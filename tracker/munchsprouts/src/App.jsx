...
// ─── COLOUR TOKENS (matching original MunchSprouts palette) ──────────────────
const C = {
  bgMain: '#f3f0fa',
  bgPurple: '#ede8f7',
  bgGreen: '#e8f7ee',
  ...
  const paths = {
    ...
    baby: (
      <>
        ...
        <path d="M8 14c-3 1.5-5 3.5-5 5h18c0-1.5-2-3.5-5-5" />
      </>
    ),
    chef: (
      <>
    ...
    leaf: (
      <>
        <path d="M2 22c1-5 4-9 8-10 1 3 3 5 5 6-2 1-4 3-5 6" />
        <path d="M22 2s-8 2-10 10C8 16 6 19 2 22c3-1 7-3 10-8 2 1 4 1 6-1-1-3-1-7 4-11z" />
      </>
    ),
    apple: (
      <>
        <path d="M12 20.94c1.5 0 2.75-.67 4-2 1.5-1.67 2-3.5 2-5.44C18 9 15.87 7 13.5 7c-.87 0-1.5.2-2 .5-.5-.3-1.13-.5-2-.5C7.13 7 5 9 5 13.5c0 1.94.5 3.77 2 5.44 1.25 1.33 2.5 2 4 2z" />
        <path d="M12 7V3" />
      </>
    ),
    utensils: (
    ...
    bean: (
      <>
        <path d="M10.5 22C6.5 22 2 17.52 2 13c0-3 1.9-5.5 5-6.5 1.3-.4 3.1.1 4.3 1.3C13.4 9.9 16 11 18 11c2 0 4-1 4-3-2 0-4-1-4-3 0-1.1.9-2 2-2" />
        <path d="M14.5 22c2-1.5 3.5-4 3.5-7 0-3-2-5-5-5" />
      </>
    ),
    sprout: (
      <>
        ...
        <path d="M9.5 9.4c1.1.8 1.8 2.2 2.3 3.7-2 .4-3.5.4-4.8-.3-1.2-.6-2.3-1.9-3-4.2 2.8-.5 5 .3 5.5.8z" />
        <path d="M14.1 6a7 7 0 0 0-1.1 4c1.9-.1 3.3-.6 4.3-1.4 1-1 1.6-2.3 1.7-4.6-2.7.1-4.9 1-4.9 2z" />
      </>
    ),
  };
  const d = paths[name];
...
function CatIcon({ category, size = 18, color }) {
  const cfg = CATEGORIES.find((c) => c.value === category);
  return (
    <Icon
      name={CAT_ICON_MAP[category] || 'utensils'}
      size={size}
      color={color || cfg?.color || C.mutedText}
    />
  );
}
            ...
            color:
              t.type === 'error'
                ...
                : t.type === 'warning'
                  ? C.warningStroke
                  : '#2e7d52',
            border: `1.5px solid ${t.type === 'error' ? '#e07070' : t.type === 'warning' ? C.warningStroke : C.primaryGreenLight}`,
            borderRadius: 12,
            padding: '10px 16px',
...
const btnOutline = {
  ...
  border: `2px solid ${C.primaryPurple}`,
  borderRadius: 12,
  padding: '9px 16px',
  fontFamily: FONT_BODY,
  fontWeight: 700,
...
const btnDanger = {
  ...
  color: C.warningStroke,
  border: `2px solid ${C.warningStroke}`,
  borderRadius: 10,
  padding: '8px 14px',
  fontFamily: FONT_BODY,
  fontWeight: 700,
...
function Card({ children, style: s = {}, danger = false }) {
  return (
    <div
      style={{
        background: danger ? '#fde8e8' : C.bgPurple,
        border: `2px solid ${danger ? '#e07070' : C.borderLight}`,
        borderRadius: 16,
        padding: 18,
...
function SectionTitle({ children, icon }) {
  return (
    ...
    >
      {icon && <Icon name={icon} size={17} color={C.primaryPurple} />}
      <h3
        style={{
          margin: 0,
          fontFamily: FONT_HEAD,
...
function FoodForm({ onSubmit, initial = {}, buttonLabel = 'Add to Log' }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Field label="Food or Drink Name">
        <input
          type="text"
          value={form.name}
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <button
                ...
                style={{
                  background: sel ? c.color : C.bgPurple,
          ...
          {REACTIONS.map((r) => {
            ...
            return (
              <button
                key={r.value}
                type="button"
                onClick={() => set('reaction', r.value)}
                style={{
                  background: sel ? r.bg : C.bgPurple,
                  border: `2px solid ${sel ? r.border : C.border}`,
                  borderRadius: 10,
                  padding: '9px 4px',
                  cursor: 'pointer',
                  fontFamily: FONT_BODY,
      ...
      >
        <input
          ...
          style={{ width: 17, height: 17, accentColor: C.primaryPurple }}
        />
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      {child ? (
        <div
          style={{
            background: `linear-gradient(135deg, ${C.primaryPurple} 0%, ${C.secondaryPurple} 100%)`,
            borderRadius: 20,
            padding: '22px 24px',
            color: C.white,
            display: 'flex',
      ...
      {favourites.length > 0 && (
        <Card>
          <SectionTitle icon="starFill">Favourites</SectionTitle>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {favourites.map((k) => {
              const cat =
                CATEGORIES.find((c) => c.value === groups[k].category) ||
                CATEGORIES[7];
              return (
                <span
                  key={k}
            ...
            {Object.entries(catBreakdown)
              ...
              .map(([cat, count]) => {
                ...
                return (
                  ...
                  >
                    <CatIcon category={cat} size={17} color={cfg.color} />
                    <div style={{ flex: 1 }}>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <span>{cat}</span>
                        <span style={{ color: C.mutedText }}>{count}</span>
                      </div>
                      <div
                        style={{
        ...
        <Card>
          ...
          >
            ...
            <button
              ...
              style={{ ...btnOutline, padding: '6px 12px', fontSize: 10 }}
            >
              View All
            </button>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {recent.map((e) => (
              ...
              >
                <CatIcon category={e.category} size={19} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {e.name}
                    {e.favourite && (
                      <Icon name="starFill" size={11} color="#d4a017" />
                    )}
                  </div>
                  <div
                    style={{
...
function LogPage({ foodLog, onEdit, onDelete, onToggleFavourite }) {
  ...
  if (filterReaction)
    keys = keys.filter((k) =>
      groups[k].attempts.some((a) => a.reaction === filterReaction),
    );
  if (sortBy === 'date-desc')
    keys.sort(
      (a, b) =>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {keys.map((key) => {
        ...
        const cat =
          CATEGORIES.find((c) => c.value === g.category) || CATEGORIES[7];

        return (
          <div
            key={key}
            style={{
              background: hasAllergy ? '#fde8e8' : C.white,
              border: `2px solid ${hasAllergy ? '#e07070' : C.borderLight}`,
              borderRadius: 16,
              overflow: 'hidden',
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  {g.name}
                  {hasFav && <Icon name="starFill" size={12} color="#d4a017" />}
                  {hasAllergy && (
                    <span
                      style={{
                        background: '#fde8e8',
            ...
            {isOpen && (
              <div style={{ borderTop: `2px solid ${C.borderLight}` }}>
                {g.attempts.map((a, i) => (
                  <div
                    key={a.id}
                    style={{
                      padding: '12px 16px',
                      background: i % 2 === 0 ? C.bgPurple : C.white,
                      borderBottom:
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
                        <button
                          onClick={() => onToggleFavourite(a.id)}
                          style={{
                            ...btnSecondary,
                            padding: '7px 9px',
                            display: 'flex',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {children.map((child) => {
        ...
        return (
          <Card
            key={child.id}
            style={{
              border: `2px solid ${isActive ? C.primaryPurple : C.borderLight}`,
              background: isActive ? C.bgPurple : C.white,
            }}
          >
            <div
              style={{
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                >
                  Born {formatDate(child.dob)} · {weeks} weeks ({months} months)
                </div>
                {child.weaningStart && (
                  <div
                    style={{
      ...
      <Modal open={adding} onClose={() => setAdding(false)} title="Add Child">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Field label="Weaning Start Date">
            <input
              type="date"
              value={form.weaningStart}
              onChange={(e) => set('weaningStart', e.target.value)}
              style={inputStyle}
            />
          </Field>
          <button
        ...
        {editing && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Field label="Weaning Start Date">
              <input
                type="date"
                value={editing.weaningStart || ''}
                onChange={(e) =>
                  setEditing((p) => ({ ...p, weaningStart: e.target.value }))
                }
                style={inputStyle}
              />
            </Field>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          >
            Create a free MunchSprouts account to access our full library of
            100+ nutritionist-approved BLW recipes.
          </p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button
              style={{ ...btnPrimary, width: 'auto', padding: '10px 22px' }}
            >
              Create Free Account
            </button>
            <button style={{ ...btnOutline, padding: '10px 18px' }}>
              Sign In
            </button>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                <span
                  style={{
                    ...
                    color: C.primaryGreen,
                    border: `1.5px solid ${C.primaryGreenLight}`,
                    borderRadius: 999,
                    padding: '2px 9px',
                    fontSize: 10,
                    fontWeight: 700,
                ...
                >
                  <Icon name="clock" size={9} color={C.mutedText} />
                  {r.time}
                </span>
                {r.tags.map((t) => (
                  <span
                    ...
                    style={{
                      background: C.bgWarning,
                      color: C.warningStroke,
                      border: `1.5px solid ${C.warningStroke}`,
                      borderRadius: 999,
                      padding: '2px 9px',
                      fontSize: 10,
                      fontWeight: 700,
...
// ─── EXPORT MODAL ─────────────────────────────────────────────────────────────
function ExportModal({ open, onClose, foodLog, activeChild }) {
  const [fmt, setFmt] = useState('csv');
  const name = activeChild?.name || 'Log';

  const dl = (content, type, filename) => {
  ...
  const exportCSV = () => {
    let csv = 'Date,Food,Category,Form,Reaction,Favourite,Notes\n';
    foodLog.forEach((f) => {
      csv += `"${f.date}","${f.name}","${f.category || ''}","${f.form || ''}","${f.reaction || ''}","${f.favourite ? 'Yes' : 'No'}","${(f.notes || '').replace(/"/g, "'")}"\n`;
    });
  ...
  return (
    <Modal open={open} onClose={onClose} title="Export Data">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {activeChild && (
          ...
          >
            ...
            >
              ...
              {foodLog.length} entries for {name}
            </div>
          </div>
        )}
        <Field label="Format">
          ...
          >
            {[
              ...
              <button
                ...
                onClick={() => setFmt(val)}
                style={{
                  background: fmt === val ? C.bgPurple : C.white,
                  border: `2px solid ${fmt === val ? C.primaryPurple : C.border}`,
                  borderRadius: 12,
                  padding: '12px 10px',
                  cursor: 'pointer',
                  fontFamily: FONT_BODY,
                  fontWeight: 700,
                  fontSize: 12,
                  color: fmt === val ? C.primaryPurple : C.mutedText,
                  display: 'flex',
                  flexDirection: 'column',
                  alignItems: 'center',
                  gap: 6,
        ...
        >
          {fmt === 'csv'
            ? 'Great for sharing with your health visitor or opening in Excel / Google Sheets.'
        ...
        <button
          ...
          onClick={fmt === 'csv' ? exportCSV : exportJSON}
        >
          <Icon name="download" size={14} color={C.white} /> Download{' '}
          {fmt.toUpperCase()}
        </button>
      </div>
    </Modal>
...
export default function MunchSproutsApp() {
  ...
  const addFood = (form, err) => {
    if (!form) {
      toast(err || 'Please fill in required fields.', 'warning');
      return;
    }
    ...
    toast(
      existing.length === 0
        ? `Added "${form.name}" to the log`
        : `"${form.name}" — attempt #${existing.length + 1}`,
    );
    setPage('log');
  };
  const editFood = (updated, err) => {
    if (!updated) {
      toast(err || 'Please fill required fields.', 'warning');
      return;
    }
    setFoodLog((p) =>
  ...
  const toggleFav = (id) =>
    setFoodLog((p) =>
      p.map((f) => (f.id === id ? { ...f, favourite: !f.favourite } : f)),
    );
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            >
              <Icon name="export" size={15} color={C.primaryPurple} />
            </button>
          </div>
        </header>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  padding: isAdd ? '6px 0 8px' : '10px 0 8px',
                  display: 'flex',
                  flexDirection: 'column',
                  alignItems: 'center',
                  gap: 3,
